# MongoDB Sharding + Replication + Redis Caching
...
4. Проверьте количество документов:
```shell
# Общее количество
docker compose exec -T mongos mongosh --port 27017 --quiet <<EOF
use somedb
...
6. Проверьте работу кеширования:
```shell
# Первый запрос (должен быть медленным)
time curl http://localhost:8080/helloDoc/users

# Второй запрос (должен быть быстрым <100мс)
time curl http://localhost:8080/helloDoc/users
```

7. Проверьте статус Redis:
```shell
...
- Кеширование включено для эндпоинта `/helloDoc/users`
- Первый запрос выполняется медленно (обращение к MongoDB)
- Последующие запросы выполняются быстро (<100мс) благодаря Redis
- Кеш автоматически обновляется при изменении данных